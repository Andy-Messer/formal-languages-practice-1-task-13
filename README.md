...
> 1. Первое на что стоит обратить внимание, что алгоритм буква за буквой разбирает реглярное выражение, 
...
> 2. И так мы построили автомат за O(n), где n = | reg_expr |,
     ...
     входной строки (возможно можно было придумать какое-нибдь ДП P.s. не придумал... ).
...
>  Для каждой такой строки запустимся по автомату с помощью BFS без пометок в used!!! Это необходимо, т.к.
     ...
     Чтобы прочитать какое-то слово требуется пройти, как минимум k состояний, а значит Ассимптотика такого BFS
